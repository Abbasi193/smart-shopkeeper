...
echo $_SESSION['user'];?></strong></span><br>
    ...
    <div class="w3-bar-block">
      ...
      <a href="bills.php" class="w3-bar-item w3-button w3-padding active"><i class="fa fa-history fa-fw"></i>  History</a>
  ...
  <div class="w3-main " style="margin-left:300px;margin-top:60px;">
    <div class="w3-container w3-half" style="padding-top:22px">
    
    <h1 class="w3-panel">History</h1>
    <form action="bills.php">
    <h3><label>Search previous orders by Id</label></h3>
    <input id='id1' oninput=redo() type="number" name='id' class="w3-input w3-border" min=0 validate>
    <br>
    <input id='btn1' onclick=check() type="submit" name='submit' class="w3-button w3-green" value="Search">
    <br>
    </form>
      <?php
        if (isset($_GET['submit']))
        { $id=htmlspecialchars($_GET['id']);
          $sql = "SELECT sum(price) as total FROM bills where orderId=$id;";
          $result = $con->query($sql);
          $row=$result->fetch_assoc();
          $total=$row["total"];

          $sql = "SELECT Orderdate as d FROM orders where orderId=$id;";
          $result = $con->query($sql);
          $row=$result->fetch_assoc();
          $d=$row["d"];


      $sql = "SELECT productName,price,quantity FROM bills where orderId=$id;";
      $result = $con->query($sql);

      if ($result->num_rows > 0) {
        echo "<br><ul class='w3-ul  w3-leftbar w3-border-blue'>
        
        <li>Bill # :&nbsp $id</li>
        <li>Date :&nbsp $d</li>
        <li>Total :&nbsp $total</li>
        </ul><br>";
        echo "<table class='w3-table-all w3-card-4'><tr  class='w3-blue'><th>Product Name</th><th>Price</th><th>Quantity</th></tr>";
 ...
 <div id='id012' class='w3-modal'>
        <div style='width:300px;height:100px' class='w3-modal-content w3-card-4 w3-container w3-red'>
            ...
            <span class='w3-container'>Error: Please enter valid value</span>
        </div>
      </div>
