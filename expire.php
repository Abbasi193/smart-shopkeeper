...
echo $_SESSION['user'];?></strong></span><br>
    ...
    <div class="w3-bar-block">
      ...
      <a href="expire.php" class="w3-bar-item w3-button w3-padding active"><i class="fa fa-bell fa-fw"></i> Expiring Items</a>      
  ...
  <div class="w3-main " style="margin-left:300px;margin-top:60px;">
    ...
    <h1 class="w3-container w3-padding">Expiring Items</h1>
      <?php

      $sql = "select productName,barCode,price,quantity,expiryDate,DATEDIFF(expiryDate,SYSDATE()) as p FROM products where DATEDIFF(expiryDate,SYSDATE())<30 order by p asc;";
      $result = $con->query($sql);

      if ($result->num_rows > 0) {
        echo "<table class='w3-table-all w3-card-4'><tr  class='w3-blue'><th>Product Name</th><th>Bar Code</th><th>Price</th><th>Quantity</th><th>Expiry Date</th><th>Status</th></tr>";
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          echo "<tr class=".$display."><td>" . $row["productName"] . "</td><td>" . $row["barCode"] . "</td><td>" . $row["price"] . "</td><td>" . $row["quantity"] . "</td><td>" . $row["expiryDate"] ."</td><td>".$display2. "</td></tr>";
        }
        echo "</table>";
      } else {
        echo "0 results";
